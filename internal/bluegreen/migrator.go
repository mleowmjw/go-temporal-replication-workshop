...
// The fake implementation simulates schema state via a column registry so
...
// so that QueryCheck can detect missing columns.
//
// Initial schema matches the workshop inventory.customers table:
// id, email, full_name, status, created_at, search_key, phone (after expand).
type FakeDatabaseMigrator struct {
	mu         sync.Mutex
	columns    map[string]bool // "table.column" or just "column" for simplicity
	ReadOnly   bool
	ExecutedSQL []string
...
// applyDDLLocked parses simple ADD COLUMN and DROP COLUMN statements and
// updates the column registry. Must be called with mu held.
func (f *FakeDatabaseMigrator) applyDDLLocked(stmt string) error {
	...
	if m := dropRe.FindStringSubmatch(stmt); len(m) == 2 {
		...
		if !f.columns[col] {
			// IF EXISTS silences this; we mirror that behaviour.
...
func (f *FakeDatabaseMigrator) validateColumnRefsLocked(query string) error {
	// Extract identifiers from common SQL patterns.
	// This is intentionally simple: extract word tokens that appear after
	// SELECT, SET col =, or in INSERT column lists, and check them.
	cols := extractColumns(query)
...
// It handles the most common patterns found in the AppSimulator queries.
...
func cleanIdent(s string) string {
	...
	// Skip function calls like count(*), max(id), etc.
	if strings.ContainsAny(s, "()") {
		return ""
	}
	// strip table prefix (inventory.customers.col -> col)
